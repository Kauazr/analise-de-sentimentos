import os
import sqlite3 # MUDANÇA: Importado sqlite3 em vez de pymysql
...
# MUDANÇA: Configuração do Banco de Dados para SQLite
...
# MUDANÇA: Conexão com o Banco de Dados SQLite
def conectar_db():
    try:
        conn = sqlite3.connect(DB_NAME)
        # Permite acessar colunas por nome (similar ao DictCursor)
        conn.row_factory = sqlite3.Row
        # Habilita o suporte a chaves estrangeiras, que é desligado por padrão no SQLite
        conn.execute("PRAGMA foreign_keys = ON;")
        print("DEBUG: Conectado ao SQLite com sucesso.")
        return conn
    except sqlite3.Error as err:
        print(f"❌ ERRO DB: Falha ao conectar ao banco de dados SQLite. Detalhes: {err}")
...
# MUDANÇA: Funções de DB atualizadas para SQLite (marcador '?' em vez de '%s')
...
# (Nenhuma alteração significativa na classe AppSentimentos, pois a lógica de UI permanece a mesma)
class AppSentimentos:
    def __init__(self, root_window):
        # ... (código __init__ existente sem alterações) ...
        ...
        if not carregar_recursos_globais():
            self.root.destroy()
            return
        # A verificação de arquivo de DB acontece na conexão
        if not os.path.exists(DB_NAME):
            messagebox.showinfo("Banco de Dados não encontrado", 
                                f"O arquivo de banco de dados '{DB_NAME}' não foi encontrado.\n"
                                f"Por favor, execute o script 'popular_banco.py' primeiro para criar e popular o banco de dados.")
            self.root.destroy()
            return
    ...
    def setup_estilos(self):
        # ... (código setup_estilos existente sem alterações) ...
    ...
    def setup_ui(self):
        # ... (código setup_ui existente sem alterações) ...
    ...
    def carregar_dados_iniciais_ui(self):
        # ... (código carregar_dados_iniciais_ui existente sem alterações) ...
    ...
    def criar_aba_avaliar_produto(self):
        # ... (código criar_aba_avaliar_produto existente sem alterações) ...
    ...
    def atualizar_produtos_combobox(self, event=None):
        # ... (código atualizar_produtos_combobox existente sem alterações) ...
    ...
    def analisar_e_salvar_avaliacao_produto(self):
        # ... (código analisar_e_salvar_avaliacao_produto existente sem alterações) ...
    ...
    def criar_aba_relatorio_produtos(self):
        # ... (código criar_aba_relatorio_produtos existente sem alterações) ...
    ...
    def gerar_e_mostrar_relatorio_produtos(self):
        # ... (código gerar_e_mostrar_relatorio_produtos existente sem alterações) ...
        ...
        ax.tick_params(axis='y', colors=FG_PRIMARY, labelsize=8)
    ...
    def criar_aba_frases_pesquisa(self):
        # ... (código criar_aba_frases_pesquisa existente sem alterações) ...
    ...
    def analisar_e_salvar_frase_pesquisada(self):
        # ... (código analisar_e_salvar_frase_pesquisada existente sem alterações) ...
    ...
    def criar_aba_relatorio_geral(self):
        # ... (código criar_aba_relatorio_geral existente sem alterações) ...
    ...
    def gerar_e_mostrar_relatorio_geral(self):
        # ... (código gerar_e_mostrar_relatorio_geral existente sem alterações) ...
        ...
        for label in labels:
            if label == 'Positivo':
                pie_colors.append(ACCENT_SUCCESS)
            elif label == 'Negativo':
                pie_colors.append(ACCENT_WARNING)
            elif label == 'Neutro':
                pie_colors.append('#888888')
            else:
                pie_colors.append(ACCENT_INFO)
        fig = Figure(figsize=(6, 5), dpi=100)
        fig.patch.set_facecolor(BG_PRIMARY)
        ax = fig.add_subplot(111)
        wedges, texts, autotexts = ax.pie(sizes, labels=None, autopct='%1.1f%%', startangle=140, colors=pie_colors, pctdistance=0.85)
...
if __name__ == "__main__":
    ...
    # A verificação da existência da conexão já é feita no __init__
    if hasattr(app, 'conn') and app.conn is not None:
        root.mainloop()
    else:
